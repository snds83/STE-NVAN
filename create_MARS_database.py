...
# python create_MARS_database.py --data_dir MARS --info_dir MARS\MARS_evaluation\info --output_dir MARS_database
...
if __name__ == '__main__':
    ...
    # Train
    train_imgs = []
    data_dir = os.path.join(args.data_dir, 'bbox_train', 'bbox_train')
    ids = sorted(os.listdir(data_dir))
    print(len(ids))
    if len(ids) < 2:
        raise ValueError(f'NO IMAGE FOUND IN {ids}')

    for id_ in ids:
        images = sorted(os.listdir(os.path.join(data_dir, id_)))
        for image in images:
            if is_image_file(image):
                train_imgs.append(os.path.abspath(os.path.join(data_dir, id_, image)))
    train_imgs = np.array(train_imgs)
    np.savetxt(os.path.join(args.output_dir, 'train_path.txt'), train_imgs, fmt='%s', delimiter='\n')
    # Test
    test_imgs = []
    data_dir = os.path.join(args.data_dir, 'bbox_test', 'bbox_test')
    ids = sorted(os.listdir(data_dir))
    for id in ids:
